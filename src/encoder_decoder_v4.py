...
# I recommend adding higher thresholds later for FP reduction
EVAL_THRESHOLDS = [0.5, 0.6, 0.7, 0.75]
...
def downsample_block(x, filters, name_prefix):
    feat = conv_block(x, filters, name_prefix=f"{name_prefix}_feat")

    down = layers.MaxPooling2D(
        pool_size=(2, 2),
...
def encoder_decoder_v4(input_shape=(256, 256, 3)):
    inputs = layers.Input(shape=input_shape, name="input")

    # Encoder
    skip1, x = downsample_block(inputs, 8, "enc1")
    skip2, x = downsample_block(x, 16, "enc2")
    skip3, x = downsample_block(x, 32, "enc3")

    # Bottleneck
    x = conv_block(x, 64, "bottleneck")

    # Decoder
    x = upsample_block(x, skip3, 32, "dec1")
    x = upsample_block(x, skip2, 16, "dec2")
    x = upsample_block(x, skip1, 8, "dec3")
...
def save_model_config():
    config = {
        ...
        "architecture": {
            "type": "encoder_decoder_v4",
            "encoder_1": [
                "SeparableConv2D(3x3,8)+ReLU",
                "SeparableConv2D(3x3,8)+ReLU",
                "MaxPooling2D(2x2)"
            ],
            "encoder_2": [
                "SeparableConv2D(3x3,16)+ReLU",
                "SeparableConv2D(3x3,16)+ReLU",
                "MaxPooling2D(2x2)"
            ],
            "encoder_3": [
                "SeparableConv2D(3x3,32)+ReLU",
                "SeparableConv2D(3x3,32)+ReLU",
                "MaxPooling2D(2x2)"
            ],
            "bottleneck": [
                "SeparableConv2D(3x3,64)+ReLU",
                "SeparableConv2D(3x3,64)+ReLU"
            ],
            "decoder_1": [
                "UpSampling2D(2x2)",
        ...
        "loss": "bce_dice_loss",
        "training_metric": "dice_coefficient",
        "batch_size": BATCH_SIZE,
        "epochs": EPOCHS,
        "prediction_threshold": THRESHOLD,
        "evaluation_thresholds": EVAL_THRESHOLDS,
        "callbacks": [
            "ModelCheckpoint",
            "ReduceLROnPlateau",
            "CSVLogger"
...
def evaluate_multiple_thresholds(y_true, y_pred_prob, thresholds, meta_df=None, split_name="val", fold_name=None):
    summary_rows = []
...
def train_and_evaluate_fold(fold_num: int):
    ...
    print(
        f"Fold {fold_num} prediction stats -> "
        f"min: {val_pred.min():.6f}, "
        f"max: {val_pred.max():.6f}, "
        f"mean: {val_pred.mean():.6f}"
    )

    threshold_summary_df = evaluate_multiple_thresholds(
        y_true=y_val,
    ...
    threshold_summary_df.to_csv(
        MODEL_RESULTS_DIR / f"{MODEL_NAME}_fold_{fold_num}_threshold_sweep.csv",
        index=False
    )
    ...
    return best_model, val_per_image_df, val_summary, history.history, threshold_summary_df
...
def evaluate_final_test_with_model(model, model_tag="best_fold"):
    ...
    print(
        f"Final test prediction stats -> "
        f"min: {test_pred.min():.6f}, "
        f"max: {test_pred.max():.6f}, "
        f"mean: {test_pred.mean():.6f}"
    )

    test_threshold_summary_df = evaluate_multiple_thresholds(
        y_true=y_test,
    ...
    return test_per_image_df, test_summary, test_threshold_summary_df
...
def main():
    ...
    for fold_num in range(1, N_FOLDS + 1):
        model, val_per_image_df, val_summary, history_dict, threshold_summary_df = train_and_evaluate_fold(fold_num)

        fold_summaries.append(val_summary)
        all_val_per_image.append(val_per_image_df)
        all_threshold_summaries.append(threshold_summary_df)
    ...
    val_summary_with_global = pd.concat(
        [val_summary_df, global_row],
        ignore_index=True
    )
    ...
    all_val_per_image_df = pd.concat(
        all_val_per_image,
        ignore_index=True
    )
    ...
    all_threshold_summaries_df = pd.concat(
        all_threshold_summaries,
        ignore_index=True
    )

    all_threshold_summaries_df.to_csv(
        MODEL_RESULTS_DIR / f"{MODEL_NAME}_all_threshold_sweeps.csv",
        index=False
    )
    ...
    if best_fold_model is not None:
        test_per_image_df, test_summary, test_threshold_summary_df = evaluate_final_test_with_model(
            best_fold_model,
        ...
        if test_per_image_df is not None and test_summary is not None:
            ...
            if test_threshold_summary_df is not None:
                test_threshold_summary_df.to_csv(
                    MODEL_RESULTS_DIR / f"{MODEL_NAME}_final_test_threshold_sweep.csv",
                    index=False
                )
        else:
