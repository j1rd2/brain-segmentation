...
def upsample_block(x, skip, filters, name_prefix):
    ...
    x = layers.Conv2D(filters, (3, 3), padding="same", activation="relu", name=f"{name_prefix}_conv")(x)
    return x
...
def encoder_decoder_V3(input_shape=(256, 256, 3)):
    ...
    # Bottleneck
    x = conv_block(x, 32, "bottleneck")

    # Decoder
    x = upsample_block(x, skip2, 16, "dec1")
    x = upsample_block(x, skip1, 8, "dec2")
...
def save_model_config():
    config = {
        ...
        "architecture": {
            "type": "encoder_decoder_v2",
            "encoder": [
                "Conv(3x3,8)+ReLU",
                "Conv(3x3,8)+ReLU",
                "StridedConv(3x3,8,stride=2)"
            ],
            "encoder_2": [
                "Conv(3x3,16)+ReLU",
                "Conv(3x3,16)+ReLU",
...
def train_and_evaluate_fold(fold_num: int):
    ...
    # Useful debug to see if probabilities are collapsing
    print(f"Fold {fold_num} prediction stats -> min: {val_pred.min():.6f}, max: {val_pred.max():.6f}, mean: {val_pred.mean():.6f}")
...
def evaluate_final_test_with_model(model, model_tag="best_fold"):
    ...
    print(f"Final test prediction stats -> min: {test_pred.min():.6f}, max: {test_pred.max():.6f}, mean: {test_pred.mean():.6f}")
