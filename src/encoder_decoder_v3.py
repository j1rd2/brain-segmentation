...
LEARNING_RATE = 1e-3
THRESHOLD = 0.5
EVAL_THRESHOLDS = [0.5, 0.3, 0.25, 0.2]
SEED = 42
...
def upsample_block(x, skip, filters, name_prefix):
    x = layers.UpSampling2D((2, 2), name=f"{name_prefix}_upsample")(x)
...
def encoder_decoder_v3(input_shape=(256, 256, 3)):
    inputs = layers.Input(shape=input_shape, name="input")

    # Encoder
    skip1, x = downsample_block(inputs, 16, "enc1")
    skip2, x = downsample_block(x, 32, "enc2")

    # Bottleneck
    x = conv_block(x, 64, "bottleneck")

    # Decoder
    x = upsample_block(x, skip2, 32, "dec1")
    x = upsample_block(x, skip1, 16, "dec2")
...
def save_model_config():
    config = {
        ...
        "architecture": {
            "type": "encoder_decoder_v3",
            "encoder_1": [
                "Conv(3x3,16)+ReLU",
                "Conv(3x3,16)+ReLU",
                "StridedConv(3x3,16,stride=2)"
            ],
            "encoder_2": [
                "Conv(3x3,32)+ReLU",
                "Conv(3x3,32)+ReLU",
                "StridedConv(3x3,32,stride=2)"
            ],
            "bottleneck": [
                "Conv(3x3,64)+ReLU",
                "Conv(3x3,64)+ReLU"
...
def train_and_evaluate_fold(fold_num: int):
    ...
    model = encoder_decoder_v3(input_shape=(IMAGE_SIZE[0], IMAGE_SIZE[1], 3))
    model.compile(
    ...
    print(
        ...
        f"min: {val_pred.min():.6f}, max: {val_pred.max():.6f}, mean: {val_pred.mean():.6f}"
    )
...
def evaluate_final_test_with_model(model, model_tag="best_fold"):
    ...
    print(
        ...
        f"min: {test_pred.min():.6f}, max: {test_pred.max():.6f}, mean: {test_pred.mean():.6f}"
    )
...
def main():
    ...
    all_threshold_summaries_df = pd.concat(all_threshold_summaries, ignore_index=True)
